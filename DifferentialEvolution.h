...
namespace de {
    ...
    public:
        struct Constraints {
            explicit Constraints(double lower = 0.0, double upper = 1.0, bool isConstrained = false) :
                    lower(lower),
                    upper(upper),
            ...
            // create constraints from a pair
            explicit Constraints(std::pair<double, double> pair) :
                    lower(pair.first),
                    upper(pair.second),
                    isConstrained(true) {
                ...
                for (int i = 0; i < m_numberOfParameters; i++) {
                    if (m_constraints[i].isConstrained) {
                        distribution = std::make_shared<std::uniform_real_distribution<double>>(
                                std::uniform_real_distribution<double>(m_constraints[i].lower, m_constraints[i].upper));
                    } else {
                        distribution = std::make_shared<std::uniform_real_distribution<double>>(
            ...
            for (int i = 0; i < iterations; i++) {
                ...
                if (m_terminationCondition) {
                    if (m_terminationCondition(*this)) {
                        if (verbose) {
                            std::cout << "Terminated due to positive evaluation of the termination condition."
                                      << std::endl;
                        }
                        return;
                    }
